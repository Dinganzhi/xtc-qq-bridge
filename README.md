...
|   |-- pngtool.py           # 纯标准库 PNG 编码（截图裁块发图片用）
|   |-- imgtool.py           # 纯标准库图片解码（PNG/GIF/JPEG）+ 相似度（核对表情原图用）
|   `-- deduplicate.py       # LRU 去重 + 文件化回声过滤
|-- emoji_store.py           # 表情原图：表情包按名字 + 缓存按**像素核对**（防"货不对板"）
|-- tools/
...
xiaotiancai:
  ...
  ui:
    ...
    blind_send: true                         # 先手打字：按上次成功发送的坐标直接输入（~1 秒出现在输入框）
...
# ---------- 表情包（仅 小天才 -> QQ 单向） ----------
emoji:
  forward_image: true          # 手表发来的表情当图片发到 QQ（失败自动退回文字）
  caption: true                # 图片是否带 [时间] [昵称] 内容 说明
...
| **输入文字看起来很慢** | 文字输入本身不慢：实机实测 `am broadcast ADB_INPUT_TEXT` **0.23s**、IME 状态查询 0.18s、`input tap` 0.26s，把一段话打进输入框总共 **≈0.75 秒**。感觉慢的是它周边的"读界面"（每次 3~4 秒）与"恢复窗口/唤醒屏幕"：① 发送链路的 dump 次数已从 4~5 次降到 1 次；② 有发送排队时轮询会让路（不再等轮询那次 dump）；③ IME 状态查询改为 60 秒缓存；④ 启动 App 的每个策略不再白等 25 秒；⑤ 把 WSA 窗口保持可见、别最小化（窗口被最小化时前台会变成 `com.microsoft.windows.homeapp`，每次都要把 App 拉回来）。详见「5. 发送结果确认与发送速度」 |
...
| **转发的表情/贴纸"货不对板"（发的不是手表上那张）** | 已修复：缓存目录里同时有聊天照片、头像和**别的贴纸**，99 个表情名里还有 55 个重名，"按时间/形状猜"必然出错（实测把"小猫流汗"发成了"弹吉他的乌龟"）。现在会抠下**界面上这张气泡的截图**当基准，逐个候选比像素，**够像（≥0.80）才发**；都不像就发那张气泡截图（静止但一定对）；补发老消息又拿不到基准图时**不猜**。见「表情包」小节 |
| **日志里出现 `[表情包] 缓存里没有像界面这张的原图`** | 说明缓存里确实没有这张的原始文件（或原文件是解不了的 WebP），本次会发**界面气泡截图**（静止一帧）——内容一定是对的。想每次都拿到动图原图，需要该表情来自本地表情包（见上一条） |
...
- **表情包（仅 小天才 → QQ 单向）**：手表发来的表情/贴纸会被**当图片**发到 QQ
  （QQ → 小天才 依旧是文字，不做图片）。取图按优先级三级：
  1. **读 App 里的原文件**（推荐路径）：小天才 App 的**外部**数据目录 adb shell 就能读
     （**不需要 root**；内部 `/data/data` 不可读、App 非 debuggable 也用不了 `run-as`）。两条来源：
     * 表情包解包目录 `<ext>/files/xtcdata/telwatch/weichat/emoji/newEmoji/<包>/<序>/<名>/big/<code>`，
       名字索引在同目录 `desc.json`（**UTF-16**）里：`{"code":"tiancaituQ_003","desc":"爱你"}`
       —— 实测 99 个名字，都是 240x240 的 GIF/PNG，名字在本包里唯一时直接命中；
     * 图片缓存 `<ext>/cache/big_image/<pkg>/<ver>/<bucket>/<hash>.cnt`（Glide 缓存，扩展名统一 `.cnt`，
       但**内容是原始格式**）：表情在消息显示时被写进来，能从里面拿到**多帧循环 GIF**（动画完整保留）。
  2. **按气泡位置截图**：拿不到可信原图时的兜底（只有一帧，动图会变静图）。
  3. 都失败就发"表情X"文字 —— **绝不因为表情丢消息**。

  **为什么必须"按像素核对"**：缓存目录里同时躺着聊天照片、头像、**别的贴纸**，而
  实测 99 个名字里有 55 个在多套表情包里重名 —— 光靠"文件名/时间/形状"猜一定会错
  （真实故障：手表发"小猫流汗"，转发出去变成"弹吉他的乌龟"）。所以现在的做法是：
  * 先抠下**界面上这张气泡的截图**当基准（`utils/imgtool.py` 纯标准库解码）；
  * 逐个候选（表情包文件 + 缓存文件）压成 10x10 网格比像素（颜色差 + 亮度结构相关性）；
  * **够像（≥0.80）才发**，实测"同一张"能到 0.95 上下、缓存里最像的无关图片只有 0.63；
  * 没有候选够像 -> 就发那张气泡截图（静止但一定是对的），**绝不硬发一张像不像都不知道的图**；
  * 补发**老消息**又拿不到基准图时，连"缓存里刚写进来的图"也不猜（这正是当初发错的场景）。

  实现细节：`EmojiStore`（`emoji_store.py`）跑在**轮询线程**里（那一刻缓存刚写入、气泡位置才准），
  先按**文件头**判断格式/尺寸/是否动图（GIF 看帧数与 NETSCAPE 块、PNG 看 `acTL`、WebP 看 `ANIM`），
  过大（>600px）或超大（>512KB）的候选直接跳过 —— 既避免把聊天照片误当贴纸，也避免去解码
  1~2MB 的大照片（纯 Python 解一张 2000x3000 的 JPEG 要十几秒，曾把找一张表情拖到 80 秒以上）；
  候选按"刚写过 -> 消息时间附近 -> 其余按时间倒序"的顺序核对，命中 ≥0.90 就收手；
  WebP 解不了就不参与核对（宁可用截图，也不硬发）。
  开关：`emoji.forward_image`（默认开）、`emoji.caption`（图片是否带 `[时间] [昵称] 内容` 说明）、
  `emoji.from_app_data`（关掉就只用截图）、`emoji.cache_recent_secs`（缓存时间窗，默认 45 秒）。
  注意这是**单向**的，且插件需一并更新（桥接通过 `/api/forward` 多带一个 `image_b64` 字段，
  插件用 `base64_image()` 发出；QQ 侧按图片实际格式渲染，GIF 会动）。
...
| 等轮询让出操作锁 | 0~4 秒（轮询那 3~4 秒的 dump 做完才轮到发送） | 0 秒：有发送排队时轮询**先让路**（最多 30 秒，漏读的消息由撞库补发兜底） |
| 打字前的等待（"QQ 发消息到文字出现在输入框"） | 等锁 + 读界面 ≈ 4~10 秒 | **≈ 1 秒**：`ui.blind_send` 先手打字 —— 按上次成功发送留下的坐标**直接点输入框、广播注入、点发送**，不占锁也不读界面；随后用一次 dump 复核。门闩：近 15 秒内轮询确认过在聊天页 **且标题就是目标联系人**（防止盲点发错聊天）；复核失败会清掉缓存并退回稳妥流程，只有"输入框仍留有内容"才允许重发 |
| 切 IME 状态查询 | 每次注入 1 次 `settings get`（0.18s） | 缓存 60 秒（切输入法/注入失败时作废） |
| App 不在前台时启动 | 每个启动策略白等 **25 秒**前台确认 | 单策略最多 6 秒；命令自己报错（`not exported`/`SecurityException`）立刻换下一套 |
| **合计** | **4~5 次 dump ≈ 20 秒+**（还与轮询抢锁，实测 22~23 秒） | **1 次 ≈ 4~5 秒**（首次发送或布局变化后走稳妥流程，约 2 次） |

**"输入文字"本身有多快**（实机逐项实测，`adb shell` 一次往返 0.11s）：

| 动作 | 实测 |
|---|---|
| `settings get secure default_input_method`（IME 状态） | 0.18s（现在按 60 秒缓存） |
| `am broadcast ADB_INPUT_TEXT`（真正的"输入文字"） | **0.23s** |
| `input tap`（点输入框/发送按钮） | 0.26s |
| 广播后等 IME 提交 | 0.35s（`_TEXT_RETRY_SLEEP`） |
| **合计：把一段文字打进输入框** | **≈ 0.75 秒**（`ui.fast_send` 快路径不再为它多 dump 一次） |

也就是说：**输入文字本身不到 1 秒**；感觉慢的部分是"读界面"（每次 3~4 秒）和
"恢复窗口/唤醒屏幕"。实测日志里最慢的两种情形与输入无关：

- WSA 窗口被最小化（前台变成 `com.microsoft.windows.homeapp`）→ 每次发送都要先把 App
  拉回前台（`am start -n` 实测 0.46s、`monkey` 0.97s，但窗口恢复本身要几秒）；
- 屏幕闲置息屏 → `uiautomator` 报 `null root node` → 先唤醒再重试（每次 ~10 秒）。
  对策：把 WSA 窗口保持可见（可挪到屏幕边上，别最小化），`adb.keep_awake: true` 已默认开启。
