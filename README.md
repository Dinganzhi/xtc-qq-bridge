...
| `/小天才 历史消息 <条数>` | 查看小天才最近 N 条对话（1-100，默认 20）。数据来自**本地消息库**（不滚动界面）：桥接启用后自动积累的真实对话，已剔除发送成功等系统提示与 `/小天才` 命令；日期为明确数字（如昨天→`09-01`），形如 `[09-01 21:41] 发送方: 内容` |
...
**历史消息（本地消息库）**：`/小天才 历史消息` 不滚动读界面，而是读 `data/msg_log.json`——
桥接在转发手表消息、把 QQ 消息发进小天才时自动归档真实对话；发送成功等系统提示与 `/小天才` 命令不入库。
日期由归档时间戳输出为明确数字（昨天 → `09-01`，前天 → `08-31`），行格式 `[MM-DD HH:MM] 发送方: 内容`。
消息库自桥接启用起积累（重启不丢；清空该文件即重新开始）。

## 小天才侧命令（在模拟器的小天才聊天里直接输入，由桥接程序执行）
...
| 命令 | 作用 |
|---|---|
| `/小天才 历史消息 <条数>` | 查看小天才最近 N 条对话（1-100，默认 20），结果写回聊天（数据同 QQ 侧：本地消息库，明确日期 `MM-DD`，无系统提示/命令） |
