...
# ---------- 表情包（仅 小天才 -> QQ 单向） ----------
emoji:
  forward_image: true          # 手表发来的表情按气泡截图当图片发到 QQ（失败自动退回文字）
...
- **表情包（仅 小天才 → QQ 单向）**：手表发来的表情/贴纸会被**按气泡位置截图**后当图片发到 QQ
  （QQ → 小天才 依旧是文字，不做图片）。为什么用截图：贴纸是 App 内部资源（可能在 apk/so 或网络里），
  ADB 拿不到原图；而贴纸在屏幕上就是它本身，按气泡区域裁剪即得原图（实测 120×120、干净无 UI）。
  实现细节：`screencap` 取原始 RGBA 像素（不用 `-p`，避免为了解码 PNG 引入 Pillow），
  抠图 + 编码 PNG 全用标准库（`utils/pngtool.py`）；截图**在轮询线程里做**——那一刻气泡位置才是准的。
  安全网：截图/编码/发送任何一步失败都自动退回发"表情X"文字，绝不因为表情丢消息；
  开关 `emoji.forward_image`（默认开）、`emoji.caption`（图片是否带 `[时间] [昵称] 内容` 说明）。
  注意：这是**单向**的，且插件需一并更新（桥接通过 `/api/forward` 多带一个 `image_b64` 字段）。
